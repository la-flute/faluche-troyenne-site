...
export default (state = initialState, action) => {
  ...
  switch (action.type) {
    ...
    case REMOVE_USER_TO_BEDROOM:
      ...
      bedroom.users = bedroom.users.filter(user => user.id !== action.payload.userId)
      bedrooms[index] = bedroom
      return {
        ...state,
        bedrooms
...
export const fetchBedrooms = () => {
  return async (dispatch, getState) => {
    ...
    try {
      const res = await axios.get('bedrooms', { headers: { 'X-Token': authToken } })
...
export const joinBedroom = (id) => {
  return async (dispatch, getState) => {
    ...
    try {
      const res = await axios.post(`bedrooms/${id}/join`, {}, { headers: { 'X-Token': authToken } })
      if (res.status === 200) {
        let { user } = getState().user
        user.bedroomId = id
        dispatch({ type: SET_USER, payload: user })
        dispatch({ type: ADD_USER_TO_BEDROOM, payload: { bedroomId: id, user} })
        dispatch(
          notifActions.notifSend({
...
export const leaveBedroom = (id) => {
  return async (dispatch, getState) => {
    ...
    try {
      const res = await axios.post(`bedrooms/${id}/leave`, {}, { headers: { 'X-Token': authToken } })

      if (res.status === 200) {
        let { user } = getState().user
        user.bedroomId = null
        dispatch({ type: SET_USER, payload: user })
        dispatch({ type: REMOVE_USER_TO_BEDROOM, payload: { bedroomId: id, userId: user.id} })
        dispatch(
          notifActions.notifSend({
...
export const addBedroom = (data) => {
  return async (dispatch, getState) => {
    ...
    try {
      const res = await axios.post(`bedrooms`, data, { headers: { 'X-Token': authToken } })
...
export const removeBedroom = (id) => {
  return async (dispatch, getState) => {
    ...
    try {
      const res = await axios.delete(`bedrooms/${id}`, { headers: { 'X-Token': authToken } })
